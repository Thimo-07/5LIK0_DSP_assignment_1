...
function X = fouriertransform_fixpt_1(x, TF, fp_dat, fp_tf, step)
    % make sure x and the TF lookup table use the
    % fixed-point representation

    % TODO: This currently goes wrong because we convert to fixed-point on line 19 and 20
    %       We then split in half and go deeper. We should not convert at lower level I think.
    %
    %       Goal: At lower level we can have smaller number of bits left of fraction, we know its [-1, 1]
    %             As we come higher, we need more bits for [0 - 16]. We can have same fraction, or increase that
    %             aswell
    ...
    if N <= 1
        ...
        evenDFT = fouriertransform_fixpt_2(even, TF_fp, fp_dat, fp_tf, 2*step);
        oddDFT = fouriertransform_fixpt_2(odd, TF_fp, fp_dat, fp_tf, 2*step);
